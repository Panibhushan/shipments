 ...
 * all business logic to ItemsService - Populate the view model and set flash
 * attributes for redirects
 *
 * This controller intentionally contains NO business logic. Validation,
 * persistence, and domain rules all live in ItemsService.
 *
 * Item composite PK convention: itemId_customerId_itemUom (e.g.
 * "SKU001_CUST01_EACH"), referred to throughout as itemCustomerUomId.
 */
@Controller
public class ItemsController {
	...
	/**
	 * Valid units of measure for items. Defined once here as a constant so the same
	 * list is reused across all handlers without re-instantiating it on every
	 * request.
	 */
	...
	public ItemsController(ItemsService itemsService, ShipmentsService shipmentsService,
			CustomersService customersService, AuditsService auditsService) {
	...
	/**
	 * Filters the items list using the submitted form values. If all three filters
	 * are "ALL", redirects to the unfiltered list to avoid a redundant query.
	 */
	@GetMapping("/items/showItemsByFilter")
	public String showItemsByFilter(@RequestParam(required = false, defaultValue = "ALL") String customerId,
			@RequestParam(required = false, defaultValue = "ALL") String itemId,
			@RequestParam(required = false, defaultValue = "ALL") String itemUom, Model model) {

		log.info("GET /items/showItemsByFilter → customerId={}, itemId={}, itemUom={}", customerId, itemId, itemUom);
	...
	/**
	 * Renders the create-item form with active customers and UOM options
	 * pre-loaded.
	 */
	 ...
	 *
	 * Flow: 1. Reject immediately if the itemCustomerUomId already exists. 2. Run
	 * field-level validation via the service. 3. On errors → redirect back to the
	 * form with error flash attributes (pre-populating fields so the user doesn't
	 * retype everything). 4. On success → create the item and redirect with a
	 * success message.
	 *
	...
	public String saveItem(@ModelAttribute Items item, RedirectAttributes redirectAttributes) {
		...
		if (!errors.isEmpty()) {
			...
			redirectAttributes.addFlashAttribute("msg", "Created Item: " + itemId + "!"
					+ " &nbsp;&nbsp;<a href='/items/showItemDetails/" + itemCustomerUomId + "'>View Item Details</a>");
	...
	/**
	 * Renders the view/edit page for a specific item. Redirects to the items list
	 * with an error message if the composite key does not match any existing
	 * record.
	 *
	 * The composite PK is split into its components to produce a meaningful error
	 * message (showing itemId, customerId, and UOM separately).
	 */
	 ...
	 *
	 * Flow: 1. Reject immediately if the item no longer exists (defensive check).
	 * 2. Run field-level validation via the service. 3. On errors → redirect back
	 * to the edit page with error flash attributes. 4. On success → update the item
	...
	public String updateItem(@RequestParam String itemCustomerUomId, @RequestParam String itemStatus,
			@RequestParam String itemName, @RequestParam String itemUom, @RequestParam String itemDescription,
			RedirectAttributes redirectAttributes) {
		...
		// ── 2. Validation ─────────────────────────────────────────────────────
		List<String> errors = itemsService.validateItemUpdate(itemDescription, itemStatus, itemName);
		...
		if (!errors.isEmpty()) {
			...
			// ── 3. Persist ────────────────────────────────────────────────────
			itemsService.updateItem(itemCustomerUomId, itemDescription, itemStatus, itemName);
	...
	/**
	 * Deletes an item by its composite PK. Redirects to the items list with a
	...
	public String deleteItem(@PathVariable String itemCustomerUomId, RedirectAttributes redirectAttributes) {
		...
		String message = itemsService.validateBeforeDeleting(itemCustomerUomId, itemId, customerId, itemUom);

		if (!message.trim().equals("")) {
			log.warn("deleteItem() → validateBeforeDeleting: itemCustomerUomId={}, message={}", itemCustomerUomId,
					message);
	...
	public List<Items> getCustomerItems(@RequestParam String customerId) {
		...
		if (itemsListResult == null || itemsListResult.isEmpty()) {
			...
			log.info("getCustomerItems() success → customerId={}: {}", customerId, itemsListResult);
		}

		return itemsListResult;
	}

	@GetMapping("/items/viewItemAudit/{itemCustomerUomId}")
	public String viewItemAudit(@PathVariable String itemCustomerUomId, Model model) {

		List<Audits> auditDetails = auditsService.getAuditDetailsList(itemCustomerUomId);
		...
		log.info("/items/viewItemAudit/itemCustomerUomId={}  → audit={}", itemCustomerUomId, auditDetails);

		if (auditDetails.isEmpty() || auditDetails == null)
			model.addAttribute("audit", "");
		else {
			model.addAttribute("auditDetails", auditDetails);
			model.addAttribute("auditForEntity", "Item: "+itemId+", Uom: "+itemUom+", Customer: "+customerId);
		}
