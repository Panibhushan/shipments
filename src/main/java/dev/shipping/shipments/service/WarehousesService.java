 ...
 *
 * Responsibilities: - CRUD operations for Warehouses and their linked addresses
 * - Input validation for both new warehouse creation and updates - Dynamic JPQL
 * query building for the warehouse filter/list view - Populating the
 * edit-warehouse view model
 *
 * Address handling: Addresses are stored in a separate table and referenced by
 * a hash-based PK (built from the address field values). This deduplicates
 * identical addresses across warehouses. Updating an address therefore creates
 * a new address record (with a new hash) rather than mutating the old one.
 *
...
public class WarehousesService {
	...
	/**
	 * Valid status values for a warehouse. Used in both create and update
	 * validation.
	 */
	...
	public WarehousesService(WarehousesRepository warehousesRepo, AddressRepository addressRepo,
			AddressService addressService, AuditsRepository auditsRepo) {
	...
	/**
	 * Returns the human-readable warehouse name for a given ID. Throws
	 * NoSuchElementException if the warehouse does not exist — caller should verify
	 * existence first.
	 */
	 ...
	 *
	 * Attributes set: - warehouse – the warehouse entity - options – status
	 * dropdown values (Active / Disabled) - selectedStatus – pre-selects the
	 * warehouse's current status - fullWarehouseAddress– formatted single-line
	 * address string for display - fullAddressToEdit – raw Address entity for the
	 * editable address form
	 *
	 * Throws RuntimeException if the warehouseId does not exist (caller must verify
	 * first).
	 */
	 ...
	 *
	 * Rules enforced: - Warehouse ID: alphanumeric + hyphens only, 3–5 characters -
	 * Warehouse Name: 5–30 characters (trimmed) - Warehouse Status: must be
	 * "Active" or "Disabled"
	 *
	...
	public List<String> validateNewWarehouse(Warehouses warehouse) {
		...
		if (!errors.isEmpty()) {
			log.warn("validateNewWarehouse() → {} validation error(s) for warehouseId={}: {}", errors.size(),
					warehouseId, errors);
		}
	...
	/**
	 * Validates fields when updating an existing warehouse. Warehouse ID cannot be
	 * changed on update, so only name and status are checked.
	 *
	 * Rules enforced: - Warehouse Name: 5–30 characters (trimmed) - Warehouse
	...
	public String createWarehouse(Warehouses warehouse) {
		...
		String savedWarehouseId = warehousesRepo.save(warehouse).getWarehouseId();
		log.info("createWarehouse() completed → warehouseId={}", savedWarehouseId);
		return savedWarehouseId;
	}

	/**
	 * Creates a warehouse and then immediately links a delivery address to it.
	 *
	 * Address deduplication: MyCustomUtils.buildAddressFields() hashes the address
	 * field values to produce a deterministic PK. If an identical address already
	 * exists in the DB, the existing record is reused rather than creating a
	 * duplicate.
	 * 
	 * @param warehouse
	 *
	 * @return "WAREHOUSE_CREATED_SUCCESSFULLY_WITH_ID: {id}" on success, or the
	 *         exception message string on unexpected failure.
	 */
	@Transactional
	public String createWarehouseWithAddress(String warehouseId, Address warehouseAddress) {
		...
		try {
			...
			warehousesRepo.findById(warehouseId).ifPresent(warehouse -> {
				...
				log.debug("createWarehouseWithAddress() → linked addressId={} to warehouseId={}", addressId,
						warehouseId);
			});
			...
			String auditData = buildWarehouseChangesObjectForAudit_CREATE(warehouse, new ArrayList<AuditFieldChange>());
			log.info("createWarehouse() auditData = {}", auditData);
			Audits audit = MyCustomUtils.setFieldsForAudit("WAREHOUSE", warehouseId, "CREATE", auditData, "SYSTEM");
			auditsRepo.save(audit);
			log.info("createWarehouse() audit={} saved for → warehouseId={}", audit.toString(), warehouseId);

			return "WAREHOUSE_CREATED_SUCCESSFULLY_WITH_ID: " + warehouseId;

		} catch (Exception e) {
			log.error("createWarehouseWithAddress() → unexpected error for warehouseId={}: {}", warehouseId,
					e.getMessage(), e);
			return e.getMessage();
		}
	}
	...
	/**
	 * Updates the name and status of an existing warehouse. Throws RuntimeException
	 * if the warehouseId does not exist.
	 */
	@Transactional
	public void updateWarehouse(String warehouseId, String warehouseName, String warehouseStatus) {
		...
		// correctly
		String auditData = buildWarehouseChangesObjectForAudit_UPDATE(warehouse, warehouseName, warehouseStatus);
		log.info("updateWarehouse() auditData = {}", auditData);
		...
		Audits audit = MyCustomUtils.setFieldsForAudit("WAREHOUSE", warehouseId, "MODIFY", auditData, "ADMIN");
		auditsRepo.save(audit);
		log.info("updateWarehouse() audit={} saved for → warehouseId={}", audit.toString(), warehouseId);
	}

	public String buildWarehouseChangesObjectForAudit_UPDATE(Warehouses warehouse, String warehouseName,
			String warehouseStatus) {
		List<AuditFieldChange> changes = new ArrayList<>();

		if (!warehouse.getWarehouseName().equals(warehouseName)) {
			changes.add(new AuditFieldChange("Description", warehouse.getWarehouseName(), warehouseName));
		}

		if (!warehouse.getWarehouseStatus().equals(warehouseStatus)) {
			changes.add(new AuditFieldChange("Status", warehouse.getWarehouseStatus(), warehouseStatus));
		}
	...
	public void buildWarehouseChangesObjectForAudit_UPDATE_OnlyAddress(String warehouseId, String currentAddressId, String newAddress) {
		List<AuditFieldChange> changes = new ArrayList<>();
		String currentAddress = MyCustomUtils.getFormattedAddress(addressRepo.findById(currentAddressId).get());

		changes.add(new AuditFieldChange("Address", currentAddress.replaceAll("<br />", " "),
				newAddress.replaceAll("<br />", " ")));

		ObjectMapper mapper = new ObjectMapper();
		String auditData = mapper.writerWithDefaultPrettyPrinter().writeValueAsString(changes);
		Audits audit = MyCustomUtils.setFieldsForAudit("WAREHOUSE", warehouseId, "MODIFY", auditData,
				"ADMIN");
		auditsRepo.save(audit);
		log.info("updateWarehouse() audit={} saved for → warehouseId={}", audit.toString(), warehouseId);
	...
	public String buildWarehouseChangesObjectForAudit_CREATE(Warehouses warehouse, List<AuditFieldChange> changes) {
		changes.add(new AuditFieldChange("Warehouse Id", "", warehouse.getWarehouseId()));
		changes.add(new AuditFieldChange("Name", "", warehouse.getWarehouseName()));
		changes.add(new AuditFieldChange("Warehouse Short Address", "", warehouse.getWarehouseShortAddress()));
		changes.add(new AuditFieldChange("Status", "", warehouse.getWarehouseStatus()));
		changes.add(new AuditFieldChange("Created At", "", warehouse.getCreatedAt()));
		changes.add(new AuditFieldChange("Modified At", "", warehouse.getModifiedAt()));

		String address = MyCustomUtils.getFormattedAddress(addressRepo.findById(warehouse.getAddressId()).get());

		changes.add(new AuditFieldChange("Address", "", address.replaceAll("<br />", " ")));
	 ...
	 *
	 * Deletion is blocked if inventory exists for the warehouse, because deleting a
	 * warehouse with live inventory would leave orphaned inventory records. The
	 * user must first adjust all inventory to zero before deletion is allowed.
	 *
	 * @return "WAREHOUSE_DELETED" on success,
	 *         "WAREHOUSE_DELETION_FAILED_BECAUSE_INVENTORY_EXISTS" if inventory is
	 *         present.
	 */
	@Transactional
	public String deleteWarehouse(String warehouseId) {
		...
		if (!inventory.isEmpty()) {
			log.warn("deleteWarehouse() blocked: {} inventory record(s) exist for warehouseId={}", inventory.size(),
					warehouseId);
		...
		String auditData = buildWarehouseChangesObjectForAudit_DELETE(warehouse, new ArrayList<AuditFieldChange>());
		log.info("deleteWarehouse() auditData = {}", auditData);
		Audits audit = MyCustomUtils.setFieldsForAudit("WAREHOUSE", warehouseId, "DELETE", auditData, "ADMIN");
		auditsRepo.save(audit);
		log.info("deleteWarehouse() audit={} saved for → warehouseId={}", audit.toString(), warehouseId);

		return "WAREHOUSE_DELETED";
	}

	public String buildWarehouseChangesObjectForAudit_DELETE(Warehouses warehouse, List<AuditFieldChange> changes) {
		ObjectMapper mapper = new ObjectMapper();
		changes.add(new AuditFieldChange("Warehouse Id", warehouse.getWarehouseId(), ""));
		changes.add(new AuditFieldChange("Name", warehouse.getWarehouseName(), ""));
		changes.add(new AuditFieldChange("Warehouse Short Address", warehouse.getWarehouseShortAddress(), ""));
	 ...
	 *
	 * @return "ADDRESS UPDATED SUCCESSFULLY" on success, "FAILED TO UPDATE ADDRESS"
	 *         otherwise.
	 */
	@Transactional
	public String updateAddress(String warehouseId, Address address) {
		...
		// Creating a new address record (old hash no longer matches after field
		// changes)
		...
		if(warehouse.isPresent()) {
			...
			String currentAddressId = warehouse.get().getAddressId();
			...
			Address newAddress = addressRepo.findById(addressId).get();
			
			buildWarehouseChangesObjectForAudit_UPDATE_OnlyAddress(warehouseId, currentAddressId, MyCustomUtils.getFormattedAddress(newAddress).replaceAll("<br />"," " ));			
	...
	private String resolveAndPersistAddress(Address rawAddress) {
		// buildAddressFields normalises the address fields and returns Map<hash →
		// Address>
